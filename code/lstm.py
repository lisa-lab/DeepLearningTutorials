...
def init_params(options):
    params = OrderedDict()
    # embedding
    params['Wemb'] = 0.01 * numpy.random.randn(options['n_words'], options['dim_proj']).astype('float32')
    # rconv
    params = get_layer(options['encoder'])[0](options, params, prefix=options['encoder'])
    # classifier
    params['U'] = 0.01 * numpy.random.randn(options['dim_proj'], options['ydim']).astype('float32')
...
def param_init_fflayer(options, params, prefix='ff'):
    params[_p(prefix,'W')] = 0.01 * numpy.random.randn(options['dim_proj'], options['dim_proj']).astype('float32')
    params[_p(prefix,'b')] = numpy.zeros((options['dim_proj'],)).astype('float32')

    return params

def fflayer(tparams, state_below, options, prefix='rconv', **kwargs):
    return eval(options['activ'])(tensor.dot(state_below, tparams[_p(prefix,'W')])+tparams[_p(prefix,'b')])
...
def param_init_lstm(options, params, prefix='lstm'):
    ...
    params[_p(prefix,'b')] = numpy.zeros((4 * options['dim_proj'],)).astype('float32')
...
def lstm_layer(tparams, state_below, options, prefix='lstm', mask=None):
    ...
    rval, updates = theano.scan(_step, 
                                ...
                                outputs_info = [tensor.alloc(0., n_samples, options['dim_proj']),
                                                tensor.alloc(0., n_samples, options['dim_proj'])],
...
def param_init_rconv(options, params, prefix='rconv'):
    ...
    params[_p(prefix,'b')] = numpy.zeros((options['dim_proj'],)).astype('float32')
    params[_p(prefix,'GW')] = 0.01 * numpy.random.randn(options['dim_proj'], 3).astype('float32')
    params[_p(prefix,'GU')] = 0.01 * numpy.random.randn(options['dim_proj'], 3).astype('float32')
...
def train(dim_proj=100, 
    ...
    f_grad_shared, f_update = eval(optimizer)(lr, tparams, grads, x, mask, y, cost)

    print 'Optimization'
    
    kf_valid = get_minibatches_idx(len(valid[0]), len(valid[0])/valid_batch_size, shuffle=True)
    kf_test = get_minibatches_idx(len(test[0]), len(test[0])/valid_batch_size, shuffle=True)
    ...
    for eidx in xrange(max_epochs):
        ...
        kf = get_minibatches_idx(len(train[0]), len(train[0])/batch_size, shuffle=True)
        ...
        for _, train_index in kf:
            ...
            x, mask, y = prepare_data([train[0][t] for t in train_index], y, maxlen=maxlen)
...
def main(job_id, params):
    ...
    trainerr, validerr, testerr = train(saveto=params['model'][0],
                                        ...
                                        dispFreq=10,
                                        saveFreq=100000,
                                        dataset='imdb', #'stanford', #'sentiment140',
